        ...
        <div>
          ...
          {change && (
            <p className="text-sm mt-2">
              <span
                className={`font-medium ${change.positive ? "text-emerald-600" : "text-red-500"}`}
              >
                {change.positive ? "+" : ""}
                {change.value}
              </span>
              <span className="text-zinc-400 ml-1">from last week</span>
            </p>
          )}
        </div>
...
function SkeletonCard() {
  return (
    <div className="bg-white rounded-xl border border-zinc-100 p-6 animate-pulse">
      <div className="flex items-start justify-between">
        <div>
          <div className="h-4 w-28 bg-zinc-100 rounded mb-3" />
          <div className="h-9 w-20 bg-zinc-100 rounded mb-2" />
  ...
  return (
    <div className="grid grid-cols-4 gap-6">
      ...
      <MetricCard
        ...
        change={{
          value: `${metrics.noShows} no-show${metrics.noShows !== 1 ? "s" : ""} yesterday`,
      ...
      <MetricCard
        ...
        change={{
          value: `${formatCurrency(metrics.agingClaimsAmount)} aging`,
          positive: metrics.agingClaimsCount < 5,
        }}
        icon={
          <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
